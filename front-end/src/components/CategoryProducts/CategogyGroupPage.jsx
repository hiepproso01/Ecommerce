...
const CategogyGroupPage = () => {
    ...
    const handleDelete = async (id) => {
        ...
        if (result.isConfirmed) {
            try {
                ...
                Swal.fire('Đã xoá!', 'Nhóm danh mục đã được xoá.', 'success');
            } catch (error) {
                console.error('Lỗi khi xoá nhóm danh mục:', error);
                Swal.fire('Lỗi!', 'Đã có lỗi xảy ra khi xoá nhóm danh mục.', 'error');
            }
        }
    };

    const handleEditOrAdd = async (id = null) => {
        const categogyGroupItem = id
            ? categogyGroup.find((ncc) => ncc.idNhomDanhMuc === id)
            : { tenNhomDanhMuc: '', idNhomDanhMuc: '' ,hinhAnhNhomDanhMuc: ''};
        ...
        const { value: formValues } = await Swal.fire({
            ...
            preConfirm: async () => {
                ...
                let imageUrl = categogyGroupItem?.hinhAnhNhomDanhMuc || '';

                if (file) {
                    const formData = new FormData();
                    formData.append('file', file);
                    try {
                        const response = await apiClient.post('/api/nhomdanhmuc/UploadImage', formData, {
                            headers: {
        ...
        if (formValues) {
            try {
                if (id) {
                    ...
                    Swal.fire('Đã cập nhật!', 'Nhóm danh mục đã được cập nhật thành công.', 'success');
                } else {
                    await apiClient.post('/api/nhomdanhmuc/Create', formValues);
                    Swal.fire('Đã thêm!', 'Nhóm danh mục đã được thêm mới thành công.', 'success');
                }

                // Refresh danh sách nhóm danh mục
                const response = await apiClient.get('/api/nhomdanhmuc/GetAll');
                setCategogyGroup(response.data);
                setFilteredCategogyGroup(response.data); // Cập nhật danh sách lọc
            } catch (error) {
                console.error('Lỗi khi lưu nhóm danh mục:', error);
                Swal.fire('Lỗi!', 'Đã có lỗi xảy ra khi lưu nhóm danh mục.', 'error');
            }
        }
    };
    ...
    const handleSearch = (e) => {
        ...
        const filtered = categogyGroup.filter(
            (product) =>
                product.idNhomDanhMuc.toString().toLowerCase().includes(e.target.value.toLowerCase()) ||
                    ...
                    <table className="table">
                        ...
                        <tbody className="table-row-1">
                            {currentItems.map((categoryProduct) => (
                                <tr key={categoryProduct.idNhomDanhMuc}>
